'''
Запрос
формирование урла методами класса для разных категорий страниц
удаление плюсов из номеров
...
def get_info_from_page(ads_obj, regions):
    ...
    def write_info_from_page(ads_obj, regions):
        driver = webdriver_conf.func_webdriver()
        for url in ads_obj.urls_ads:
            get_info_from_page(url, ads_obj, regions, driver)
        driver.close()
